...
class PackageFormState extends ConsumerState<PackageForm> {
  ...
  // CHANGED: Process multiple photos with ML Kit Text Recognition using shared OcrParser
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          if (result.codAmount != null && result.codAmount! > 0) {
            _paymentType = 'cod_cash';
            _codCashController.text = result.codAmount!.toStringAsFixed(2);
          ...
          if (result.zone != null && result.zone!.isNotEmpty) {
            _zoneController.text = result.zone!;
          }
